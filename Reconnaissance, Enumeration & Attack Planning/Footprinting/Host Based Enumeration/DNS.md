...
- الـDNS بيحول الDomain ل IP وبيرجعو للمتصفح ويفتحلي الwebsite
...
- DNS is not encrypted, but IT professionals apply `DNS over TLS` or `DNS over HTTPS` in network protocol uses `DNSCrypt`
---
## DNS Records
...
```shell-session
0xTroj6n@htb[/htb]$ dig soa www.inlanefreight.com
...
;; MSG SIZE  rcvd: 128
```
in the email address replace dot (.) to (@)
