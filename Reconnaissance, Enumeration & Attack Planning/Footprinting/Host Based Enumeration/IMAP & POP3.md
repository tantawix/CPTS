- **IMAP** is a protocol allows online management od emails directly on the server
- the protocol is client-server and allows sync of a local email client with the mailbox on the server
- **POP3** provides listing, retrieving, and deleting emails at email server
- الفكرة الأساسية هنا إن **البروتوكول ده مش بينقل الإيميلات من السيرفر لجهازك**. هو بس بيخليك تشوفهم وتتعامل معاهم وهما لسه موجودين على السيرفر، وده اللي بيخليك تقدر تشوف نفس الإيميلات من أي جهاز تاني
- client establishes connection on port `143`
- IMAP uses text-based commands in ASCII
- IMAP works unencrypted and transmits commands, emails, or usernames and passwords in plain text, email server require establishing IMAP an encrypted IMAP session so IMAP use SSL/TLS encryption for this purpose
---
#### IMAP Commands
...
By Default ports `110` and `995` are used for POP3, and ports `143` and `993` are used for IMAP the higher ports `995`, `993` use SSL/TLS encrypt the communcation between client and server
#### Nmap

```shell-session
0xTroj6n@htb[/htb]$ sudo nmap 10.129.14.128 -sV -p110,143,993,995 -sC
...
```shell-session
0xTroj6n@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user user:p4ssw0rd
...
```shell-session
0xTroj6n@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user cry0l1t3:1234 -v
...
```shell-session
0xTroj6n@htb[/htb]$ openssl s_client -connect 10.129.14.128:pop3s
...
```shell-session
0xTroj6n@htb[/htb]$ openssl s_client -connect 10.129.14.128:imaps
