...
- بروتوكول الRDP بيسمحلنا نشوف ونتحكم في الجهاز بواجهه جرافيكيه GUI بقي مش command line زي الssh
- الrdp بيشتغل علي بورت 3389
- لازم الFirewall بتاع الشبكه وبتاع الجهاز يسمح انه يعمل اتصال RDP
- طيب دلوقتي لو عندك شبكة داخليه LAN يعني فيها اجهزة بعنوان داخليه لما تطلع علي الانترنت بيتعملها عملية NAT والip الداخلي ده بيبقي ip خارجي
- فا المشكله هنا لو السيرفر يكون ورا NAT فا هو كده مش متشاف علي الانترنت يعني مثلا العميل اللي عايز يوصل للسيرفر مش هيقدر يوصله على `192.168.1.50` لأن ده IP داخلي مش ظاهر للإنترنت
**الحل**
-الحل هنا فالport forwarding يعني بقول للراوتر أي اتصال ييجي على المنفذ الفلاني (port) من الإنترنت، دخّله للجهاز ده في الشبكة الداخلية
- الRDP شغال
---
...
```shell-session
0xTroj6n@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --script rdp*
...
```shell-session
0xTroj6n@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --packet-trace --disable-arp-ping -n
...
...SNIP...
```
- الـ**NLA (Network Level Authentication)** يعني إن السيرفر بيطلب مصادقة قبل ما يفتح جلسة RDP كاملة — العملية دي تستخدم بروتوكول **CredSSP**.
    
- لو NLA مفعّل، هتقل فرص هجمات بسيطة زي brute-force أو بعض استغلالات handshake لأن السيرفر مش بيقدم واجهة RDP كاملة للي مش مصدّق. ده شرح رسمي لطريقة عمل NLA
#### RDP Security Check - Installation

```shell-session
0xTroj6n@htb[/htb]$ sudo cpan
...
```shell-session
0xTroj6n@htb[/htb]$ git clone https://github.com/CiscoCXSecurity/rdp-sec-check.git && cd rdp-sec-check
0xTroj6n@htb[/htb]$ ./rdp-sec-check.pl 10.129.201.248
...
- ممكن تكونكت rdp من لينكس ل ويندوز عن طريق ادوات زي `xfreerdp, rdesktop,Remmina` وساعتها هتتحكم بواجهه GUI عادي
#### Initiate an RDP Session

```shell-session
0xTroj6n@htb[/htb]$ xfreerdp /u:cry0l1t3 /p:"P455w0rd!" /v:10.129.201.248
...
- الWinRM بيستخدم SOAP عشان يفتح اتصال remote مع الجهاز اللي بكونكت بيه
-  WinRM must be explicitly enabled and configured starting with Windows 10. WinRM relies on `TCP` ports `5985` and `5986` for communication, with the last port `5986 using HTTPS`
...
```shell-session
0xTroj6n@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p5985,5986 --disable-arp-ping -n
...
- في حاجه اسمها `Test-WsMan` دي عباره عن تول او Cmdlet جاهز فالpower shell وظيفته بيتأكد اذا كان الجهاز بيدعم WinRM ولا لأ وبيرجعلك معلومات عن الendpoint
```
Test-WsMan -ComputerName 10.0.0.5
```
- الـ**evil-winrm** أداة شائعة في عالم الـ pentesting على لينكس/كالي علشان تتعامل مع WinRM. بتسهل عليك فتح جلسة PowerShell على ويندوز لو معاك credentials
```shell-session
0xTroj6n@htb[/htb]$ evil-winrm -i 10.129.201.248 -u Cry0l1t3 -p P455w0rD!
...
---
## WMI
- تخيّل جهاز ويندوز كـ **بيت كبير**، وكل حاجة فيه (خدمات، برامج، إعدادات، registry، هاردوير، مستخدمين، جداول مجدولة...) ليها مفاتيح علشان نتحكم فيها.  
الـ**WMI** هو **المكتب الإداري** بتاع البيت ده: واجهة مركزية بتقدّر تقرأ أو تكتب أو تتحكم في أي حاجة تقريبًا في النظام—يعني مكان واحد تقدر تطلب منه معلومات أو تطلب منه ين
فّذ تغييرات.
- الWMI بيشتغل علي بورت 135 بس بعد ما الاتصال والسيشن تتفتح الاتصال بينتقل لبورت تاني عشوائي
## المصطلحات المهمة بسرعة

- **WMI** = Windows Management Instrumentation — تنفيذ مايكروسوفت لإدارة النظام.
    
- **CIM** = Common Information Model — نموذج معياري لكيفية تمثيل موارد الأنظمة.
    
- **WBEM** = Web-Based Enterprise Management — معيار أكبر يضم CIM; وWMI هو تطبيق للمعايير دي على ويندوز.
    
- **Repository** = قاعدة بيانات داخلية لـ WMI بتخزن المعلومات والمخططات (مثل مكتبة المكتب الإداري).
    
- **Providers** = موديولات بتقول لـWMI إزاي تتكلم مع حاجة معينة (مثلاً provider للهاردوير، و provider للـ services، إلخ).
#### WMIexec.py

```shell-session
0xTroj6n@htb[/htb]$ /usr/share/doc/python3-impacket/examples/wmiexec.py Cry0l1t3:"P455w0rD!"@10.129.201.248 "hostname"
