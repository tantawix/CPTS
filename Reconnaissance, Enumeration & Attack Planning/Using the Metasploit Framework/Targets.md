- الTargets ده بيكون نسخه او نوع os معين الميتاسبلويت بتظبط عليه طريقة الexploitation عشان تشتغل صح
#### MSF - Show Targets
...
Exploit target:
   ...
   0   Automatic
```
- خلي بالك مكتوب هنا تحت الname كلمة automatic يعني يكتشف نظام التشغيل لوحده ويظبط الاستغلال تلقائيًا
- ساعات ممكن تكون كده ف modules تانيه
0 Automatic
1 Windows 7 SP1
...
---
## Selecting a Target
- تعالي نجرب موديول تاني ونشوف الinfo
...
```
- بصلي هنا كده علي الavailable targets
```shell-session
...
Exploit targets:
   ...
   6   IE 9 on Windows 7
```
- طيب عشان نختار تارجت بقي هنكتب ايه؟
```
set target <index no.>
```
...
---
## Target Types
- ليه الtargets بتختلف عن بعض؟ لأن الاستغلال بيعتمد علي حاجه اسمها return address
- الreturn address ده عنوان فالذاكره البرنامج بعد مايخلص فانكشن يرجع يكمل تنفيذ من العنوان ده لو العنوان ده بقي صح يبقي الexploitation نجح لو غلط هيحصل crash
- العنوان بيختلف عشان بيكون في اختلاف اللغات او اختلاف نسخة البرنامج وبرضو اساليب الsecurity بين كل تارجت والتاني
###### أنواع الReturn Address
1. jmp esp
والأمر ده بيقول للميموري نفذ الكود اللي عند ESP
2. jump to register
```
jmp eax
jmp ebx
```
3. pop / pop / ret
ودي مهمتها بتنضف الstack وبعد كده ret عشان ترجع علي عنوان انت متحكم فيه4
###### الTarget بيتحدد علي أساس إيه؟
- نوع الReturn Address
- مكانه في أي DLL
- النسخة واللغه
عشان كده الtarget بنلاقي اسمه
