- الDatabase فالmsfconsole بتستخدم عشان تتبع النتائج اللي معاك
- وانت شغال علي complicated machine بيكون في نتايج scan كتير وثغرات وبورتات وخدمات وليلة كبيره اوي
- ف هنا بقي الdb بتلعب دورها وبتسجلك كل حاجه وبتخليك ترجع لأي نتيجه بسهوله وتربط بينهم
---
## Setting up the Database
#### PostgreSQL Status

```shell-session
0xTroj6n@htb[/htb]$ sudo service postgresql status
...
```shell-session
0xTroj6n@htb[/htb]$ sudo systemctl start postgresql
```
...
```shell-session
0xTroj6n@htb[/htb]$ sudo msfdb init
...
```shell-session
0xTroj6n@htb[/htb]$ sudo msfdb status
...
```shell-session
0xTroj6n@htb[/htb]$ sudo msfdb run
...
```shell-session
0xTroj6n@htb[/htb]$ msfdb reinit
0xTroj6n@htb[/htb]$ cp /usr/share/metasploit-framework/config/database.yml ~/.msf4/
0xTroj6n@htb[/htb]$ sudo service postgresql restart
...
---
## Using the Database
- بعد ما ظبطت الدنيا وبدأت الdb والدنيا تمام ممكن نستخدم حاجه اسمها Workspaces
---
#### Workspaces
- الworkspaces ممكن نشبهها بالfolders اللي جوا project مثلا
- ممكن نفصل النتايج عن بعض من خلالها برضو
- عشان نشوف الcurrent workspaces هنكتب كوماند `workspace` وعشان اتنقل بين الworkspaces هنكتب الفلاجز دول مع الكوماند `a-` عشان نضيف workspace جديده او `d-` عشان نمسح workspace
```shell-session
msf6 > workspace
...
- تعالي نجرب نعمل import ل nmap scan كده وهنستخدم كوماند اسمه `db_import` وعشان نجيب معلومات الhosts اللي عملت عليها scan هنكتب امر `hosts` او `services`
- خلي بالك يفضل ان الملف بتاع الscan result يكون بصيغة xml
#### Stored Nmap Scan

```shell-session
0xTroj6n@htb[/htb]$ cat Target.nmap
...
---
## Data Backup
- عشان نعمل backup بقي للداتا بتاعتنا هنستخدم امر `db_export`
#### MSF - DB Export
...
---
## Hosts
- اتكلمنا قبل كده علي أمر `host` وده بيستخدم عشان يعرضلنا الdb table للhosts اللي عملنا عليها scan بالhostnames بتاعتها والip والمعلومات التانيه
#### MSF - Stored Hosts
...
---
## Services
- الservices command نفس الكلام زي الhosts بتعرضلك كل المعلومات عن الservices اللي طلعتلك فالscan
...
---
## Credentials
- الأمر `creds` بتسمحلك انك تشوف الcreds اللي حصلت عليها من خلال الscan او وقت الinteractions مع الtarget host اللي بتعمل عليه scan بل احنا ممكن نضيف creds بشكل يدوي
...
---
## Loot
- ال`loot` كوماند بيجبلك الحاجات الحساسه اللي سحبتها من التارجت زي الpassword hashes او ملفات المستخدمين
#### MSF - Stored Loot
