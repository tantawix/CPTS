- الMeterpreter ده payload بيتوسع لوحده عن طريق انه بيستخدم DLL Injection عشان يتأكد ان الاتصال مع الvictim ثابت وصعب ان الAV او الIPS والIDS يكتشفه ولو رسترت الجهاز مش هيحصل اي حاجه وهيفضل الاتصال شغال
- الMeterpreter بيشتغل فالmemory بتاعت الvictim ومبيسبش اي traces او اثر وراه وده بيخليه صعب انه يتكشف
- الMeterpreter بيطلق عليه اسم siwiss army knife
- الغرض الاساسي من الmeterpreter انه يحسن من عملية الpost-exploitation اللي هو احنا اخترقنا الجهاز ف بنشوف بقي هنقدر نعمل ايه فيه سواء كان PrivEsc او Pivoting
---
## Running Meterpreter
- تعالي نعرف لما ننفذ exploit علي حد ايه البيحصل
1. اول حاجه الtarget بيexecute initial stager ايا كان بقي bind, reverse, findtag,passivex
2. الStager بيload ملف الDLL ويبدأ يعمل Injection وخلي بالك الDLL بتكون شغاله فالRAM وده اسمه Fileless execution
3. الmeterpreter بيبدأ بشكل رسمي وكل الداتا او الاوامر اللي بنفذها علي جهاز الضحيه بتكون AES-encrypted يعني كل الداتا سواء commands, output,extensions مش واضحه لأي حد بيعمل sniffing بعد كده الmeterpreter بيبعت GET request الmetasploit بتستقبله وبيعمل session binding
4. فالخطوه دي بيتم تحميل الextensions زي
`stdapi` main commands like (ls, ps, cat, netstat)
`priv` privs commands like (getsystem, hashdump)
...
```
- خلي بالك احنا عملنا هنا امر `steal_token` والكوماند ده فالويندوز بيحدد انت مين وصلاحياتك ايه انك لبست هوية process تانيه فبالتالي لو عملنا امر `getuid` هيقولك اليوزر بتاع الprocess اللي احنا سرقنا التوكين بتاعته `NT AUTHORITY\NETWORK SERVICE`
...
```
- الكوماند ده بيطلعلنا LSA Secrets ودي أسرار النظام نفسه اللي ويندوز مخزنها جوه Local Security Authority
- الsecrets دي ممكن تكون كلمات سر plain text مثلا او master key بتاع wifi password او rdp creds او saved creds
---
