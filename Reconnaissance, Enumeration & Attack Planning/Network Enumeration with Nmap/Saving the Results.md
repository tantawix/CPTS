## Different Formats
- While we run various scans, we should always save the results وده عشان كل تكنيك نحفظه في ملف لوحده كا نوع من التنسيق يعني
...
```shell-session
0xTantawyy@htb[/htb]$ sudo nmap 10.129.2.28 -p- -oA target
...
#### Normal Output
```shell-session
0xTantawyy@htb[/htb]$ cat target.nmap
...
```shell-session
0xTantawyy@htb[/htb]$ cat target.gnmap
...
#### XML Output
```shell-session
0xTantawyy@htb[/htb]$ cat target.xml
...
- لو عندي XML output ممكن بسهوله اني اعمل HTML Report يكون اوضح للقراءه
- عشان احول الملف من XML ل HTML بستخدم اداة `xsltproc`
```shell-session
0xTantawyy@htb[/htb]$ xsltproc target.xml -o target.html
```
---
